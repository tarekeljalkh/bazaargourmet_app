...
import 'package:bazaargourmet_ecommerce/features/offline_payment/domain/models/offline_payment_model.dart';
...
class OfflineCardWidget extends StatelessWidget {
  ...
  const OfflineCardWidget({super.key, required this.offlinePaymentModel, required this.index});
    ...
    return Consumer<CheckoutController>(
      builder: (context, orderProvider, _) {
        return Stack(children: [
            Padding(padding: const EdgeInsets.fromLTRB(0, Dimensions.paddingSizeDefault, Dimensions.paddingSizeDefault,
                Dimensions.paddingSizeDefault),
              child: Container(width: MediaQuery.of(context).size.width *.75,height: 180, decoration: BoxDecoration(
                color: index == orderProvider.offlineMethodSelectedIndex?Theme.of(context).cardColor :
                Theme.of(context).primaryColor.withValues(alpha:.125),
                border: index == orderProvider.offlineMethodSelectedIndex?
                Border.all(color: Theme.of(context).primaryColor.withValues(alpha:.5), width: .5): null,
            ...
            Positioned(top: 30, right: 20,
              child: Row(children: [
                Text('${getTranslated('pay_on_this_account', context)}',
