...
import 'package:bazaargourmet_ecommerce/features/coupon/domain/models/coupon_item_model.dart';
import 'package:bazaargourmet_ecommerce/features/coupon/domain/models/coupon_model.dart';
...
class CouponController extends ChangeNotifier {
  ...
  Future<void> applyCoupon(BuildContext context,String coupon, double order) async {
    ...
    _discount = 0;
    notifyListeners();
    ApiResponse apiResponse = await couponRepo!.get(coupon);
  ...
  Future<void> getCouponList(BuildContext context,int offset) async {
    _isLoading = true;
    ApiResponse apiResponse = await couponRepo!.getList(offset : offset);
    if (apiResponse.response != null  && apiResponse.response!.statusCode == 200) {
      couponList = [];
      _isLoading = false;
